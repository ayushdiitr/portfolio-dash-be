...
const calculatePortfolio = async (): Promise<PortfolioResponse> => {
  ...
  const totalInvestment = holdingsWithMarketData.reduce(
    (sum, h) => sum + h.investment,
    0
  );
  const totalPresentValue = holdingsWithMarketData.reduce(
  ...
  const bySector: Record<string, SectorGroup> = {};
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    const cached = await get<PortfolioResponse>(PORTFOLIO_CACHE_KEY);
    if (cached) {
